...
installPlugin() {
  if [ -f ${plugin_dir}/${1}.hpi -o -f ${plugin_dir}/${1}.jpi ]; then
    ...
    echo "Installing: $1"
    # added insecure option to bypass kadaster certificate on bluecoat
    curl -L --silent --insecure --output ${plugin_dir}/${1}.hpi  https://updates.jenkins-ci.org/latest/${1}.hpi
    return 0
  fi
}
